...
-- 2. Ver comentários automáticos gerados
SELECT 
  card_id,
  author_name,
  content,
...
WHERE content LIKE '%📎 Anexo adicionado%' 
   OR content LIKE '%🗑️ Anexo removido%'
ORDER BY created_at DESC;

-- 3. Ver anexos de um card específico (substitua pelo ID do seu card)
...
-- 5. Ver arquivos no storage (se possível)
